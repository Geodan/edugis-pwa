...
class GroupedArray {
    ...
    getItem(itemid) {
        return this.items.find(searchItem=>searchItem._ga_id == itemid);
    }
    getLastItemInGroup(itemid) {
        let itemIndex = this.items.findIndex(searchItem=>searchItem._ga_id == itemid);
        if (itemIndex > -1) {
            if (!this.items[itemIndex]._ga_group) {
                // item is not a group, so return item
                return this.items[itemIndex];
            }
            const group = this.items[itemIndex];
            for (let i = itemIndex - 1; i > -1; i--) {
                if (this._matchItem(group, this.items[i]) < group._ga_depth) {
                    // end of group
    ...
    _updateGroup(item, isOpen) {
        ...
        if ((groupIndex > -1) && this.items[groupIndex]._ga_group && this.items[groupIndex]._ga_visible) {
            ...
            for (let i = groupIndex - 1; i > -1; i--) {
                ...
                if (this.items[i]._ga_id && !this.items[i]._ga_open) {
