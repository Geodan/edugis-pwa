class TrafficLayer {
    constructor(traffic_url) {
        this.id = 'trafficlayer';
        this.type = 'custom';
        this.renderingMode = '3d';
		this.trafficURL = traffic_url;
		this.vertices = null;
		...
		this.map = {};
		this.gl = null;
		this.trafficPromise = this.loadTraffic();
		this.interval = null;
    }
	project(x, y) {
		const xmin = -20037508.3427892;
		const ymin = -20037508.3427892;
		const xmax = 20037508.3427892;
		const ymax = 20037508.3427892;
		const xrange = xmax - xmin;
		const yrange = ymax - ymin;
		
		return ([(x - xmin) / xrange, (y - ymax) / yrange * -1]);
	}
	loadTraffic() {
		let self = this;
		return new Promise((resolve, reject) => {
			fetch(self.trafficURL)
			...
			for (let i = 0; i < data.features.length; i++) {
				...
				*/
				...
				for (let j = 1; j < feature.geometry.coordinates.length; j++) {
					...
					let loc2_p = self.project(loc2[0], loc2[1]);//proj4(proj4('EPSG:3857'), proj4('EPSG:4326')).forward(loc2);
